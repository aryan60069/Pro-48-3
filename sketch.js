...
var muddyRoadAccidentImg, brokenCar, sadCar;
var redLightImg, greenLightImg, stopButtonImg;
var diversionImg;
var gameOverImg, restartImg;
var bg, initialBg, bgStart;
var mainCar, muddyRoad, brokenRoad, sidePathLeft, sidePathRight;
var muddyRoadGroup, brokenRoadGroup, tLightGroup, diversionGroup;
var lpl, rpl;
var tLight, stopButton;
var diversion;
var PLAY = 0;
var END = 1;
var INITIAL_STATE = 2;
...
function preload(){
  //preloading the images and sounds
   ...
   sadCar = loadImage("images/SadCar.png");
  ...
  bgStart = createSprite(500,900,400,400);
  bgStart.addImage("bg",initialBg);
  bgStart.scale = 2;
  ...
  sidePathRight.addImage("2", sidePathImg);
  ...
  stopButton = createSprite(mainCar.x-58, mainCar.y, 10,10);
...
function draw() {
  ...
  if(gameState === INITIAL_STATE){
    ...
    text("3. Move your mouse on the Pause button to stop the car on red light.", 100, 240);
    text("4. Remove your mouse from the Pause button to start the car as the red light disappears.", 100, 280);
    text("5. Prevent the car from colliding with obstacles, else you would lose the game.",100, 320);
    text("6. Press Reset in case you lose the game.",100,360);
    text("7. Press SPACE key to start the game.",100,400);
    text("8. All the Best !! Make your High Score.",100,440);
  ...
  if(gameState === PLAY){
    ...
    bg.velocityY = (1.5*frameCount/80);
    sidePathLeft.velocityY = (1.5*frameCount/80);
    sidePathRight.velocityY = (1.5*frameCount/80);
  ...
  //Ending the Game if muddy road touches main car
  if(muddyRoadGroup.isTouching(mainCar)){
   ...
   if(mouseIsOver(stopButton)){
    ...
    pButton();
    distance = Math.round(frameCount/50 + 1);
    stroke("red");
    fill("red");
    textSize(18);
    text("Distance", 2,30);
    text(":"+distance+"m" ,10,50)
  }
...
function spawnBrokenRoad(){
  ...
  mainCar.depth = mainCar.depth + 1;
  restart.depth = brokenRoad.depth;
  restart.depth = restart.depth + 1;
...
function trafficLight(){
  if(frameCount%700 === 0){
    ...
    switch(rand){
      ...
      break;
      default : tLight.addImage(greenLightImg);
  }

  if(tLight.y > 500){
    mainCar.addImage("sad",sadCar);
    mainCar.changeAnimation("sad",sadCar);
    gameOverSound.play();
    gameOver.visible = true;
    restart.visible = true;
    gameState = END;
